...
grey_mat = gr.material({0.2,0.2,0.2}, {0,0,0}, 100)

gold_shiny_mat = gr.material({0.75164, 0.60648, 0.22648}, {0.628281, 0.555802, 0.366065}, 100)
gold_mat = gr.material({0.75164, 0.60648, 0.22648}, {0,0,0}, 100)


-- red_mat = gr.material({1,0.15,0.15}, {0.1,0.1,0.1}, 25, 0)
-- green_mat = gr.material({0.1,0.9,0.1}, {0.1,0.1,0.1}, 25, 0)
...
root:add_child(s2)
if SHOW_EFFECT then
	s2:set_material(gold_shiny_mat)
else
	s2:set_material(gold_mat)
end
